...
public class SupplyControl {
    ...
    public double DailySupplyConsumption(double suppliessubtracted, int teamPace, int sizeofparty) {
        //Calculations
        if (teamPace == 1) {
            suppliessubtracted = 3 * sizeofparty;
        }
        if (teamPace == 2) {
            suppliessubtracted = 3 * 1.25 * sizeofparty;
        }
        if (teamPace == 3) {
            suppliessubtracted = 3 * 1.50 * sizeofparty;
        }
        //tests for vaild input
        if (suppliessubtracted <= 0) {
            suppliessubtracted = -1;
        }
        //37 is the theoretical max as the most in a party is 8 and at the most draw is 36
        if (suppliessubtracted > 37) {
            suppliessubtracted = -1;
        }
        {
